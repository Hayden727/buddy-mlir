...
void print_matrix_for_comparison(const float* data, size_t rows, size_t cols) {
    // For very small matrices, we can print. For larger ones, this might be too much.
    // Let's add a limit to avoid overwhelming the console.
    ...
    for (size_t r = 0; r < print_rows; ++r) {
        ...
        for (size_t c = 0; c < print_cols; ++c) {
            printf("%g", data[r * N + c]); // Use N for column stride
            if (c < print_cols - 1) {
                printf(",   ");
            }
        }
